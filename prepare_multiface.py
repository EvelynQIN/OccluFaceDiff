...
def create_processed_data(path_to_dataset, device, subject_id):

    subject_name = dataset_setting.multiface_entities[subject_id]
    
    # init face alignment detector
    face_detector = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, device=device)
    sbj = [x.path for x in os.scandir(path_to_dataset) if x.is_dir() and subject_name in x.name][0]
    ...
    for motion in os.scandir(os.path.join(sbj, "images")):
        ...
        logger.info(f"{subject_name}_{motion.name} has {null_face_cnt} / {len(frame_ids)} null faces.")
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="=subject id to get the labeled data",
    )
    path_to_dataset = 'dataset/multiface'
    device = 'cuda'
    args = parser.parse_args()
    create_processed_data(path_to_dataset, device, args.subject_id)