...
# use FiLM layer to inject diffusion timestep information
class FaceTransformerFLINTMLP(nn.Module):
    def __init__(
        self,
        arch,
        latent_dim=256, 
...
class FaceTransformerFLINTwoFiLM(nn.Module):
    def __init__(
        self,
        arch,
        latent_dim=256, 
        ...
        **kwargs):
        ...
        self.tag = 'FaceTransformerFLINTwoFiLM'
        self.input_feats = 128
        self.dataset = dataset
        self.use_mask = use_mask
        if self.use_mask:
    ...
    def forward(self, x, timesteps, image=None, lmk_2d=None, img_mask=None, lmk_mask=None, audio_input=None, force_mask=False, **kwargs):
        ...
        decoder_output = self.transformer_decoder(xseq, src_seq, tgt_mask=tgt_mask, memory_mask=memory_mask) # [seqlen, bs, d]
        output = self.outputprocess_motion(decoder_output)  # [bs, seqlen, input_nfeats]
        return output