## copied from https://github.com/TimoBolkart/TEMPEH/blob/90cc1e5c2a3d5866efb22e7a05d36295869acae6/utils/camera.py#L107
...
def load_mpi_camera(calib_fname, resize_factor=1, to_meters=True):
    ''' get camera calibrantions from fname
    ...
    camera = {
        ...
        'radial_distortion': radial_distortion,    # (2,)
...
def batch_perspective_project_wo_distortion(points, camera_intrinsics, camera_extrinsics):
    '''
    Projection of batch of 3D points into the image plane using a perspective transformation without distortion.
    Args:
        points: 3d landmarks, (bs, V, 3)
        camera_intrinsics: (bs, 3, 3)
        camera_extrinsics: [R|T], (bs, 3, 4)

    Returns:
        points_image: 2d projected images, (bs, V, 2)
    '''
    ...
    M = torch.bmm(camera_intrinsics, camera_extrinsics) # (bs, 3, 4)

    # Transformation from the world coordinate system to the image plane
    points_image = M.bmm(points_homogeneous) # (batch_size, 3, num_points)
    points_image = torch.transpose(points_image, 1, 2)  # (batch_size, num_points, 3)

    points_image_x = points_image[:,:,0] / points_image[:,:,2]
    points_image_y = points_image[:,:,1] / points_image[:,:,2]

    points_image = torch.cat((points_image_x.unsqueeze(-1), points_image_y.unsqueeze(-1)), dim=-1)   # (batch_size, num_points, 2)
    return points_image
...
def batch_perspective_project(points, camera_intrinsics, camera_extrinsics, radial_distortion, eps=1e-7):
    ...
    Args:
        ...
        radial_distortion (_type_): _description_
...
def batch_world_to_cam_project(points, camera_extrinsics):
    """3d to 2d transformation
    ...
    # Transformation from the world coordinate system to the image coordinate system using the camera extrinsic rotation (R) and translation (T)
    points_cam = camera_extrinsics.bmm(points_homogeneous) # (batch_size, 3, num_points)
 
    points_cam = torch.transpose(points_cam, 1, 2)     # (batch_size, num_points, 2) 
    return points_cam

def batch_cam_to_img_project(points, trans):
    ...
    lmk2d_reproject = batch_perspective_project_wo_distortion(points, intrin, extrin)

    return lmk2d_reproject