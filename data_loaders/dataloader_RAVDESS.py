...
class TestRAVDESSDatasetRandomOcclusion(Dataset):
    def __init__(
        self,
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_path is not None and os.path.exists(os.path.join(self.mask_path, f"{motion_id}_mask.npy")):
            ...
            mask_info = np.load(mask_path_motion, allow_pickle=True)[()]
            frame_ids = mask_info['frame_ids']
            mask_path = mask_info['mask_path']
        else:
            if seqlen <= 55:
                sid = 5
                occ_num_frames = min(35, seqlen-sid)
            else:
                sid = torch.randint(low=0, high=seqlen-55, size=(1,))[0]
                occ_num_frames = torch.randint(low=55, high=seqlen-sid+1, size=(1,))[0]
        ...
        batch['original_image'] = self._get_image_info(motion_id)
