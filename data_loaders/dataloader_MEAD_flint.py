...
import random
from utils.occlusion import MediaPipeFaceOccluder, get_test_img_occlusion_mask, RandomOcclusion
import cv2 
...
class TestMeadDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_path is not None and os.path.exists(os.path.join(self.mask_path, f"{motion_path}_mask.npy")):
            ...
            kpts = (batch['lmk_2d'].clone() * 112 + 112).long()
            batch['lmk_mask'] = self._get_lmk_mask_from_img_mask(batch['img_mask'], kpts)
        else:
...
class TestMeadDatasetRandomOcclusion(Dataset):
    def __init__(
        self,
        dataset_name, # list of dataset names
        dataset_path,
        ...
        load_audio_input=True,
        vis=False,
        mask_folder=None # dict of mask_path & frame_ids
    ):
        self.split_data = split_data
        self.fps = fps
        self.dataset = dataset_name
    ...
    def _get_image_info(self, motion_path):
        data_dict = {}
        with h5py.File(os.path.join(self.image_folder, motion_path, 'cropped_frames.hdf5'), "r") as f:
            data_dict['original_image'] = torch.from_numpy(f['images'][:]).float()
        return data_dict
    ...
    def __getitem__(self, idx):
        ...
        if self.mask_folder is not None:
            mask_info = np.load(os.path.join(self.mask_folder, f"{motion_path}_mask.npy"), allow_pickle=True)[()]
            frame_ids = mask_info['frame_ids']
            mask_path = mask_info['mask_path']
        else:
            if seqlen <= 45:
                sid = 5
                occ_num_frames = min(30, seqlen-sid)
            else:
                sid = torch.randint(low=0, high=seqlen-45, size=(1,))[0]
                occ_num_frames = torch.randint(low=45, high=seqlen-sid+1, size=(1,))[0]
            frame_ids = torch.arange(sid, sid+occ_num_frames)
            batch['lmk_mask'], mask_path = self.random_occluder.get_landmark_mask(batch['lmk_2d'], frame_ids, self.occ_type)
        
        batch['image'] = self.random_occluder.get_image_with_mask(batch['original_image'], mask_path, frame_ids)
...
def load_data(dataset, dataset_path, split, input_motion_length):
    ...
    with open(os.path.join(processed_folder, 'video_list.pkl'), 'rb') as f:
        video_list = pickle.load(f)
    motion_list = []
    for video_id, num_frames in video_list:
        # check split and motion length
