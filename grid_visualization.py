...
class GridVis:
    ...
    def _setup_renderer(self):
        self.render = SRenderY(
            ...
            v_mask=self.flame_v_mask['face']).to(self.device)
...
def main():
    ...
    if args.dataset == 'mead_25fps':
        rec_paths = {
            ...
            'faceformer': os.path.join(args.vis_folder, 'FaceFormer', exp_name, 'reconstruction'),
            'facediffuser': os.path.join(args.vis_folder, 'FaceDiffuser', exp_name, 'reconstruction'),
            'codetalker': os.path.join(args.vis_folder, 'CodeTalker', exp_name, 'reconstruction'),
            'emote': os.path.join(args.vis_folder, 'EMOTE', exp_name, 'reconstruction'),
        }
        ...
        test_video_list = load_test_data(
            ...
            sent_list=sent_list)
        shuffle_id = np.random.permutation(len(test_video_list))[:10]
