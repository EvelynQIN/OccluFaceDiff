...
class MotionTracker:
    ...
    def __init__(self, config, model_cfg, test_data, device='cuda'):
        ...
        # vis settings
        self.to_mp4 = config.to_mp4 # if true then to mp4, false then to gif w/o audio
    ...
    def track(self):
        ...
        for i in tqdm(range(num_test_motions)):
            ...
            if batch['exp'].shape[1] != self.config.n_exp:
                batch['exp'] = torch.cat([batch['exp'], torch.zeros(self.num_frames, 50)], dim=-1)
